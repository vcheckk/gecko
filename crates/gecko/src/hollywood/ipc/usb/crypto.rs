//! ```c
//! decrypted[i] = ft[(addr + i) & 7] + (raw[i] ^ sb[(addr + i) & 7])
//! ```
//!
//! ```c
//! encrypted[i] = (data[i] - ft[(addr + i) & 7]) ^ sb[(addr + i) & 7]
//! ```
//!
//! Mirrors the algorithm in FFCC at `kpad_nunchuk_keygen` (0x803D9210) and the
//! decrypt loop at `kpad_extension_crypt_buffer` (0x803DA000). The constant
//! tables here are the same ones that live in FFCC `.data5` at
//! `kpad_nunchuk_select_table` (0x80541068) and `kpad_nunchuk_perm_table`
//! (0x80541098..0x80541998 = keygen sbox + 8 round sboxes).

const ANS_TBL_1ST_PARTY: [[u8; 6]; 7] = [
    [0xa8, 0x77, 0xa6, 0xe0, 0xf7, 0x43],
    [0x5a, 0x35, 0x85, 0xe2, 0x72, 0x97],
    [0x8f, 0xb7, 0x1a, 0x62, 0x87, 0x38],
    [0x0d, 0x67, 0xc7, 0xbe, 0x4f, 0x3e],
    [0x20, 0x76, 0x37, 0x8f, 0x68, 0xb7],
    [0xa9, 0x26, 0x3f, 0x2b, 0x10, 0xe3],
    [0x30, 0x7e, 0x90, 0x0e, 0x85, 0x0a],
];

#[rustfmt::skip]
const KEYGEN_SBOX_1ST_PARTY: [u8; 256] = [
    ,
];

#[rustfmt::skip]
const SBOXES_1ST_PARTY: [[u8; 256]; 8] = [
    [
        ,
    ],
    [
        ,
    ],
    [
        ,
    ],
    [
        ,
    ],
    [
        ,
    ],
    [
        ,
    ],
    [
        ,
    ],
    [
        ,
    ],
];

#[derive(Debug, Clone, Copy)]
pub(super) struct Cipher {
    ft: [u8; 8],
    sb: [u8; 8],
}

impl Cipher {
    /// Identity cipher: what the SDK uses before any encryption key is set.
    /// Basically a nop.
    pub(super) const IDENTITY: Self = Self { ft: [0; 8], sb: [0; 8] };

    /// Reconstruct the cipher the SDK is about to use from the 16-byte
    /// extension key the guest just wrote to nunchuk register 0x40.
    ///
    /// The SDK chose a random `idx` in 0..7 that isn't transmitted in the key
    /// (see `kpad_nunchuk_keygen` 0x803D9210).
    /// We brute-force it: try every idx and pick the one whose
    /// `GenerateKeyData(rand, idx)` reproduces the trailing 6 bytes of the key.
    pub(super) fn from_extension_key(ext_key: &[u8; 16]) -> Self {
        let mut rand: [u8; 10] = ext_key[..10].try_into().unwrap();
        rand.reverse();
        let mut key: [u8; 6] = ext_key[10..].try_into().unwrap();
        key.reverse();

        for idx in 0..7 {
            if self::generate_key_data(&rand, idx) == key {
                let (ft, sb) = self::generate_tables(&rand, &key, idx);
                return Self { ft, sb };
            }
        }

        tracing::warn!(
            key = format!("{ext_key:02X?}"),
            "nunchuk encryption key did not match any idx; falling back to identity"
        );
        Self::IDENTITY
    }

    pub(super) fn encrypt(&self, data: &mut [u8], addr: u32) {
        for (i, b) in data.iter_mut().enumerate() {
            let k = ((addr + i as u32) & 7) as usize;
            *b = b.wrapping_sub(self.ft[k]) ^ self.sb[k];
        }
    }
}

fn generate_key_data(rand: &[u8; 10], idx: usize) -> [u8; 6] {
    let ans = &ANS_TBL_1ST_PARTY[idx];
    let t0: [u8; 10] = std::array::from_fn(|i| KEYGEN_SBOX_1ST_PARTY[rand[i] as usize]);

    [
        ((ans[0] ^ t0[5]).rotate_right(t0[2] as u32).wrapping_sub(t0[9])) ^ t0[4],
        ((ans[1] ^ t0[1]).rotate_right(t0[0] as u32).wrapping_sub(t0[5])) ^ t0[7],
        ((ans[2] ^ t0[6]).rotate_right(t0[8] as u32).wrapping_sub(t0[2])) ^ t0[0],
        ((ans[3] ^ t0[4]).rotate_right(t0[7] as u32).wrapping_sub(t0[3])) ^ t0[2],
        ((ans[4] ^ t0[1]).rotate_right(t0[6] as u32).wrapping_sub(t0[3])) ^ t0[4],
        ((ans[5] ^ t0[7]).rotate_right(t0[8] as u32).wrapping_sub(t0[5])) ^ t0[9],
    ]
}

fn generate_tables(rand: &[u8; 10], key: &[u8; 6], idx: usize) -> ([u8; 8], [u8; 8]) {
    let sbox_a = &SBOXES_1ST_PARTY[idx];
    let sbox_b = &SBOXES_1ST_PARTY[idx + 1];
    let a = |i: u8| sbox_a[i as usize];
    let b = |i: u8| sbox_b[i as usize];

    let ft = [
        a(key[4]) ^ b(rand[3]),
        a(key[2]) ^ b(rand[5]),
        a(key[5]) ^ b(rand[7]),
        a(key[0]) ^ b(rand[2]),
        a(key[1]) ^ b(rand[4]),
        a(key[3]) ^ b(rand[9]),
        a(rand[0]) ^ b(rand[6]),
        a(rand[1]) ^ b(rand[8]),
    ];

    let sb = [
        a(key[0]) ^ b(rand[1]),
        a(key[5]) ^ b(rand[4]),
        a(key[3]) ^ b(rand[0]),
        a(key[2]) ^ b(rand[9]),
        a(key[4]) ^ b(rand[7]),
        a(key[1]) ^ b(rand[8]),
        a(rand[3]) ^ b(rand[5]),
        a(rand[2]) ^ b(rand[6]),
    ];

    (ft, sb)
}
